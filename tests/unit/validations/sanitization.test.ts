...
describe('sanitization validation schemas', () => {
  ...
  describe('safeEmailSchema', () => {
    it('should validate and sanitize email addresses', () => {
      const validEmails = ['test@example.com', 'user.name@domain.com', 'TEST@EXAMPLE.COM  '];
  ...
  describe('createNameSchema', () => {
    it('should validate and sanitize names', () => {
      ...
      const validNames = ['John Doe', "Mary O'Connor", 'Jean-Pierre', 'Anna-Maria'];
    ...
    it('should reject names with invalid characters', () => {
      const schema = createNameSchema('Name');
      const invalidNames = ['John123', 'John@Doe', 'John.Doe', 'John_Doe', 'John/Doe'];
  ...
  describe('createJsonSchema', () => {
    ...
    it("should reject JSON that doesn't match schema", () => {
      ...
      expect(result.success).toBe(false);
      expect(
        result.error?.issues.some((issue) => issue.message.includes('Invalid JSON structure'))
      ).toBe(true);
    });
