...
import { getOIDCClient, OIDCClient, resetOIDCClient } from '@/lib/oidc/client';
  ...
  describe('handleCallback', () => {
    describe('Token Exchange', () => {
      it('should exchange authorization code for tokens with PKCE', async () => {
        ...
        const callbackUrl = new URL(
          'http://localhost:4001/api/auth/oidc/callback?code=test-code&state=test-state'
        );
        const nonce = mockResponse.claims().nonce as string;

        await client.handleCallback(callbackUrl, 'test-state', nonce, 'test-code-verifier');
      ...
      it('should handle provider errors in callback URL', async () => {
        ...
        const callbackUrl = new URL(
          'http://localhost:4001/api/auth/oidc/callback?error=access_denied&error_description=User+canceled'
        );
      ...
      it('should throw TokenExchangeError on exchange failure', async () => {
        const mockConfig = createMockOIDCConfiguration();
        vi.mocked(oauth.discovery).mockResolvedValue(mockConfig as never);
        vi.mocked(oauth.authorizationCodeGrant).mockRejectedValue(
        ...
        const callbackUrl = new URL(
          'http://localhost:4001/api/auth/oidc/callback?code=invalid&state=state'
        );
    ...
    describe('ID Token Validation - Defense in Depth', () => {
      it('should validate email claim exists', async () => {
        ...
        const callbackUrl = new URL(
          'http://localhost:4001/api/auth/oidc/callback?code=test&state=test'
        );
