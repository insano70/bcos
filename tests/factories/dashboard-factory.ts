 ...
 */
export async function createDashboardForUser(
  user: User,
  options: Partial<CreateDashboardOptions> = {}
): Promise<Dashboard> {
...
export async function deleteDashboards(dashboardIds: string[]): Promise<void> {
  ...
  await db.delete(dashboards).where(inArray(dashboards.dashboard_id, dashboardIds));
}
