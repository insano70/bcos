 ...
 */
export async function assignPermissionsToRole(roleId: string, permissionNames: PermissionName[]): Promise<void> {
  const tx = getCurrentTransaction()
 ...
 */
export async function createTestSystemRole(options: Omit<CreateRoleOptions, 'organizationId' | 'isSystemRole'> = {}): Promise<Role> {
  return createTestRole({
    ...options,
    isSystemRole: true,
  })
 ...
 */
export async function createTestRoles(count: number, baseOptions: CreateRoleOptions = {}): Promise<Role[]> {
  const roles: Role[] = []
