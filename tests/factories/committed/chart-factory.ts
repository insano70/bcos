...
export class CommittedChartFactory extends BaseFactory<CommittedChart, CreateChartOptions> {
   ...
   */
  protected async createInDatabase(options: CreateChartOptions): Promise<CommittedChart> {
    ...
    const [chart] = await this.db.insert(chart_definitions).values(chartData).returning();
  ...
  protected override async modifyOptionsForBatch(
    ...
    const name = baseOptions.chart_name ? `${baseOptions.chart_name} ${index + 1}` : undefined;
  ...
  ): Promise<CreateChartOptions | undefined> {
    if (!options.created_by) {
      throw new Error(
 ...
 * ```
 */
export async function createCommittedChart(options: CreateChartOptions): Promise<CommittedChart> {
