 ...
 *
 * NOTE: Uses committed factories because security functions use the
...
describe('Security Authentication Integration', () => {
  ...
  beforeEach(async () => {
    scopeId = `security-auth-test-${nanoid(8)}`;
  ...
  afterEach(async () => {
    // Clean up security records first (due to FK constraints - references user)
