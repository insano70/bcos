...
import { createTestUser, createInactiveTestUser } from '@/tests/factories/user-factory'
...
describe('Token Refresh Integration', () => {
  describe('Successful Token Refresh', () => {
    ...
    it('should maintain session ID on token refresh', async () => {
      ...
      const auth = await authenticateUser(email, password)
      const originalSessionId = auth.sessionId
  ...
  describe('Sliding Window Expiration', () => {
    it('should extend session on successful refresh', async () => {
      ...
      const auth = await authenticateUser(email, password)
      const initialData = await (await fetch(`${baseUrl}/api/auth/refresh`, {
        method: 'POST',
      ...
      const newCookies = await (await fetch(`${baseUrl}/api/auth/refresh`, {
        method: 'POST',
        headers: { 'Cookie': auth.cookies }
      })).headers.get('set-cookie') || ''

      const refreshData = await (await fetch(`${baseUrl}/api/auth/refresh`, {
        method: 'POST',
