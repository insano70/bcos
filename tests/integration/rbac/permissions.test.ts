...
describe('RBAC Permission System', () => {
  ...
  describe('Cross-Scope Permission Behavior', () => {
    ...
    it('should properly isolate organization-scoped permissions', async () => {
      ...
      expect((await testUserPermission(user1, 'users:read:organization', undefined, org2.organization_id)).granted).toBe(false)

      // User2 should NOT have access to org2 (no permissions)
      expect((await testUserPermission(user2, 'users:read:organization', undefined, org2.organization_id)).granted).toBe(false)
    })
  })
})
