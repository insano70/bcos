/**
 * RBAC Users Service Integration Tests
 *
 * Tests core CRUD operations and permission enforcement for users.
 */
...
describe('RBAC Users Service - Integration Tests', () => {
  ...
  describe('getUsers - Read Operations', () => {
    ...
    it('should return only own record without read permissions', async () => {
      ...
      // Create another user that should NOT be visible
      const otherUser = await createCommittedUser({
        firstName: 'Other',
        lastName: 'Person',
        scope: scopeId,
      });
      ...
      // Without read permissions, user should only see their own record (RBAC scope: 'own')
      const result = await usersService.getUsers();
      ...
      const userIds = result.map((u) => u.user_id);
      expect(userIds).not.toContain(otherUser.user_id);
    });
  });
  ...
  describe('deleteUser - Deletion Operations', () => {
    ...
    it('should deny user deletion without permissions', async () => {
      ...
      await expect(usersService.deleteUser(targetUser.user_id)).rejects.toThrow(
