/**
 * RBAC Charts Service Comprehensive Integration Tests - Using Committed Factories
 *
 * This test suite provides complete coverage of the RBACChartsService using
 * committed transactions that are visible to the service layer. Tests cover:
 ...
 * with automatic cleanup and scope isolation.
 *
 * This suite serves as the flagship model for all API testing.
 */
...
describe('RBAC Charts Service - Comprehensive Tests', () => {
  ...
  describe('getCharts - Query & Filtering', () => {
    it('should retrieve charts with real data', async () => {
      ...
      const chart1 = await createCommittedChart({
        ...
        chart_name: 'Sales Line Chart',
        chart_type: 'line',
        scope: scopeId
      })
      const chart2 = await createCommittedChart({
        created_by: user.user_id,
        chart_name: 'Revenue Bar Chart',
        chart_type: 'bar',
        scope: scopeId
      })
    ...
    it('should filter charts by is_active status', async () => {
      ...
      const userContext = await buildUserContext(user)
      const chartsService = createRBACChartsService(userContext)
      const result = await chartsService.getCharts({ is_active: true })
      ...
      result.forEach(chart => {
        if (chart.chart_definition_id === activeChart.chart_definition_id ||
            chart.chart_name.includes('test_chart_')) {
    ...
    it('should apply pagination with offset', async () => {
      ...
      const charts = []
      for (let i = 0; i < 3; i++) {
        charts.push(await createCommittedChart({
          created_by: user.user_id,
  ...
  describe('getChartById - Single Record Retrieval', () => {
    ...
    it('should deny retrieval without permissions', async () => {
      ...
      await expect(
        chartsService.getChartById(chart.chart_definition_id)
  ...
  describe('deleteChart - Deletion Operations', () => {
    ...
    it('should deny deletion without permissions', async () => {
      ...
      await expect(
        chartsService.deleteChart(chart.chart_definition_id)
      ).rejects.toThrow(PermissionDeniedError)
    })
  })
  ...
  describe('Data Validation and Business Logic', () => {
    it('should preserve complex data_source JSON structure', async () => {
      ...
      const dataSource = {
        ...
        joins: [
          { left: 'sales', right: 'inventory', on: 'product_id' }
        ]
      }
