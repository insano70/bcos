 ...
 */
export async function assignUserToOrganization(user: User, organization: Organization): Promise<UserOrganization> {
 ...
 */
export async function buildUserContext(user: User, currentOrganizationId?: string): Promise<UserContext> {
  const tx = getCurrentTransaction()
  ...
  if (roleIds.length > 0) {
    ...
    allPermissions.push(...rolePerms.map(rp => ({
  ...
  const isSuperAdmin = userRolesData.some(ur =>
    ur.role.is_system_role === true && ur.role.name === 'super_admin'
  )
  ...
  const rolesWithPermissions = userRolesData.map(ur => ({
    ...
    permissions: allPermissions.filter(p =>
      roleIds.includes(ur.role_id) // This is a simplification - in reality we'd need role-permission mapping
    ),
  }))
 ...
 */
async function getAccessibleOrganizations(userOrganizations: Organization[]): Promise<Organization[]> {
  const tx = getCurrentTransaction()
  const accessibleOrgIds = new Set(userOrganizations.map(org => org.organization_id))
