...
export class APILogger {
  ...
  constructor(request: NextRequest, module: string = 'api') {
   ...
   */
  logRateLimit(limit: number, remaining: number, resetTime: Date, action?: 'warn' | 'block'): void {
  ...
  logBusiness(
    ...
    const level = outcome === 'failure' ? 'error' : outcome === 'partial' ? 'warn' : 'info';
  ...
  logExternalAPI(
    ...
    const level = outcome === 'failure' ? 'error' : outcome === 'timeout' ? 'warn' : 'info';
 ...
 */
export function createAPILogger(request: NextRequest, module?: string): APILogger {
