...
function validateScriptContent(content: string, type: string, skipValidation?: boolean): boolean {
  ...
  if (type === 'text/javascript') {
    ...
    return allowedJSPattern.test(content) && 
...
export function NonceScript({ children, type = 'application/json', id, className, skipValidation }: NonceScriptProps) {
  const scriptNonce = useScriptNonce()
  
  // Validate content safety
  if (!validateScriptContent(children, type, skipValidation)) {
    if (process.env.NODE_ENV === 'development') {
      console.warn('NonceScript: Content failed security validation', { type, content: children.substring(0, 100) })
    }
    return null
  }
...
export function NonceStyle({ children, id, className, skipValidation }: NonceStyleProps) {
  ...
  if (!validateCSSContent(children, skipValidation)) {
    if (process.env.NODE_ENV === 'development') {
      console.warn('NonceStyle: CSS content failed security validation', { content: children.substring(0, 100) })
    }
    return null
  }
