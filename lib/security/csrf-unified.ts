    ...
    try {
      ...
      if (isAnonymousEndpoint) {
        ...
        if (!cookieToken) {
          ...
          // Record failure for security monitoring
          CSRFSecurityMonitor.recordFailure(request, 'missing_cookie_token_authenticated_endpoint', 'medium')
          return false
        }
        ...
        try {
          ...
          if (encodedPayload) {
            ...
            if (payload.type === 'anonymous') {
              ...
              CSRFSecurityMonitor.recordFailure(request, 'anonymous_token_on_protected_endpoint', 'high')
              return false
            }
            ...
            if (payload.type === 'authenticated') {
              ...
              if (!isTokenValid) {
                ...
                // Record failure for security monitoring
                CSRFSecurityMonitor.recordFailure(request, 'authenticated_token_signature_invalid', 'medium')
                return false
              }
              ...
              if (!isDoubleSubmitValid) {
                ...
                // Record failure for security monitoring
                CSRFSecurityMonitor.recordFailure(request, 'double_submit_validation_failed', 'medium')
              }
              return isDoubleSubmitValid
            }
          }
