...
import type { Practice, StaffMember, PracticeAttributes } from './practice';
import type { practices, staff_members, templates, practice_attributes } from '../db/schema';
...
export function transformPracticeAttributes(dbAttributes: typeof practice_attributes.$inferSelect): PracticeAttributes {
  ...
  if (dbAttributes.insurance_accepted) result.insurance_accepted = safeJsonParse(dbAttributes.insurance_accepted, [])
  if (dbAttributes.conditions_treated) result.conditions_treated = safeJsonParse(dbAttributes.conditions_treated, [])
  ...
  if (dbAttributes.gallery_images) result.gallery_images = safeJsonParse(dbAttributes.gallery_images, [])
