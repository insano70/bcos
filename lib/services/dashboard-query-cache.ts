...
export function generateQueryHash(
  ...
  if (runtimeFilters.advancedFilters) signature.advancedFilters = runtimeFilters.advancedFilters as unknown[];
 ...
 */
function shouldIncludeGroupByInQuery(chartType: string, groupBy?: string): boolean {
  // Currently, groupBy is transformation-only for all chart types
...
export class DashboardQueryCache {
  ...
  getStats() {
    return {
      ...
      deduplicationRate: this.stats.hits + this.stats.misses > 0
