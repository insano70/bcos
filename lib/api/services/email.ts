 ...
 * Requires Node.js runtime (nodemailer dependency)
 */
...
export class EmailService {
  ...
  async sendSystemNotification(
    ...
    const template = this.getSystemNotificationTemplate({
      message,
      details: details || {},
    });
  ...
  async sendAppointmentRequest(
    ...
    const template = this.getAppointmentRequestTemplate(formData);
   ...
   */
  private getWelcomeTemplate(vars: { firstName: string; lastName: string }): EmailTemplate {
   ...
   */
  private getEmailVerificationTemplate(vars: {
    firstName: string;
   ...
   */
  private getPracticeSetupTemplate(vars: {
    ownerName: string;
    practiceName: string;
    dashboardUrl: string;
  }): EmailTemplate {
   ...
   */
  private getSystemNotificationTemplate(vars: {
    message: string;
    details?: Record<string, unknown>;
  }): EmailTemplate {
   ...
   */
  private getAppointmentRequestTemplate(formData: {
    firstName: string;
    lastName: string;
    email: string;
    phone: string;
...
// Re-export singleton instance and factory for convenience
export { emailService, getEmailService } from './email-service-instance';
