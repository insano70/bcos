...
class APISecurityAuditor {
  ...
  extractPermissions(content: string): string[] {
    ...
    let match = permissionPattern.exec(content);
    ...
    while (match !== null) {
      ...
      match = permissionPattern.exec(content);
    }
  ...
  checkAuthentication(info: EndpointInfo, _content: string): void {
  ...
  checkInputValidation(info: EndpointInfo, content: string): void {
    ...
    if (hasMutationMethod) {
      ...
      const hasValidation = validationPatterns.some(
        (pattern) =>
  ...
  checkRateLimit(info: EndpointInfo, _content: string): void {
    // Special endpoints that need rate limiting
    const needsRateLimit =
      info.path.includes('/auth/') || info.path.includes('/upload') || info.path.includes('/api/');
  ...
  generateReport(): void {
    ...
    console.log(
      `  Protected Endpoints: ${this.endpoints.filter((e) => e.hasAuthentication).length}`
    );
