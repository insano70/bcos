...
import { useWorkItemChildren, useWorkItemComments, useWorkItemAttachments } from '@/lib/hooks/use-work-items';
...
export default function DeleteWorkItemModal({
  ...
  const { data: comments } = useWorkItemComments({ work_item_id: isOpen ? workItemId : '', limit: 1000 });
  const { data: attachments } = useWorkItemAttachments({ work_item_id: isOpen ? workItemId : '', limit: 1000 });
      ...
      <div className="px-5 py-4">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Are you sure you want to delete <strong>"{workItemSubject}"</strong>? This action cannot be undone.
          </p>
          ...
          {showImpact && hasImpact && (
            <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-left">
              <div className="flex items-start">
                <svg className="w-5 h-5 text-amber-600 dark:text-amber-400 mt-0.5 mr-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <ul className="text-sm text-amber-800 dark:text-amber-300 space-y-1">
                    {childCount > 0 && (
                      <li className="flex items-center">
                        ...
                        <span><strong>{childCount}</strong> child work item{childCount !== 1 ? 's' : ''}</span>
                      </li>
                    )}
                    {commentCount > 0 && (
                      <li className="flex items-center">
                    ...
                    {attachmentCount > 0 && (
                      <li className="flex items-center">
                        ...
                        <span><strong>{attachmentCount}</strong> attachment{attachmentCount !== 1 ? 's' : ''}</span>
                      </li>
                    )}
                  </ul>
                </div>
