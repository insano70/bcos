...
import DashboardRowBuilder, {
  type DashboardChartSlot,
  type DashboardRow,
  type RowBasedDashboardConfig,
} from './dashboard-row-builder';
...
export default function RowBasedDashboardBuilder({
  ...
  useEffect(() => {
    if (editingDashboard && availableCharts.length > 0) {
      ...
      if (editingDashboard.charts && Array.isArray(editingDashboard.charts)) {
        ...
        rows = Array.from(rowsMap.entries())
          ...
          .map(([y, charts], index) => ({
            id: `row-${y}-${index}`,
            heightPx:
              (typeof editingDashboard.layout_config?.rowHeight === 'number'
                ? editingDashboard.layout_config.rowHeight
                : 150) *
              Math.max(
                1,
                Math.max(
                  ...charts.map(
                    (c) =>
                      editingDashboard.charts?.find(
                        (ec) => ec.chart_definition_id === c.chartDefinitionId
                      )?.position_config?.h || 2
                  )
                )
              ),
        ...
        console.log('🔄 Converted to row-based layout:', {
          originalCharts: editingDashboard.charts.length,
          rows: rows.length,
        });
      }
      ...
      const layoutFilterConfig = editingDashboard.layout_config?.filterConfig as
  ...
  const saveDashboard = async () => {
    ...
    try {
      ...
      console.log(
        `💾 ${isEditMode ? 'Updating' : 'Creating'} row-based dashboard:`,
        dashboardDefinition
      );
      ...
      const _response = await apiClient.request(url, {
        method,
      ...
      setToastMessage(
        `Failed to ${isEditMode ? 'update' : 'save'} dashboard: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
      setToastType('error');
      setShowToast(true);
    } finally {
      ...
      <div className="p-6">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setDashboardConfig((prev) => ({ ...prev, dashboardDescription: e.target.value }))
              }
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setFilterConfig((prev) => ({ ...prev, enabled: e.target.checked }))
                }
            ...
            <div className="space-y-4 pl-4 border-l-2 border-violet-200 dark:border-violet-800">
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFilterConfig((prev) => ({ ...prev, showDateRange: e.target.checked }))
                    }
                    className="w-4 h-4 text-violet-600 border-gray-300 rounded focus:ring-violet-500"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFilterConfig((prev) => ({ ...prev, showOrganization: e.target.checked }))
                    }
                    className="w-4 h-4 text-violet-600 border-gray-300 rounded focus:ring-violet-500"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFilterConfig((prev) => ({ ...prev, showPractice: e.target.checked }))
                    }
                    className="w-4 h-4 text-violet-600 border-gray-300 rounded focus:ring-violet-500"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setFilterConfig((prev) => ({ ...prev, showProvider: e.target.checked }))
                    }
                    className="w-4 h-4 text-violet-600 border-gray-300 rounded focus:ring-violet-500"
                  />
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-3">
                  💡 Default values are applied when the dashboard loads. Users can override with
                  URL parameters.
                </p>
              </div>
            </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100">
              Dashboard Layout ({dashboardConfig.rows.length} row
              {dashboardConfig.rows.length !== 1 ? 's' : ''})
            </h3>
        ...
        <div className="flex justify-between">
          ...
          <button
            ...
            disabled={
              isSaving || !dashboardConfig.dashboardName.trim() || dashboardConfig.rows.length === 0
            }
      ...
      <DashboardPreviewModal
        ...
        dashboardConfig={{
          ...
          charts: dashboardConfig.rows.flatMap((row, rowIndex) =>
            row.charts
              .filter((chart) => chart.chartDefinition && chart.chartDefinitionId)
              .map((chart, chartIndex) => ({
