...
import { useUpdateDataSource, type DataSource, type UpdateDataSourceData } from '@/lib/hooks/use-data-sources';
...
export default function EditDataSourceModal({ 
  ...
  const updateDataSourceMutation = dataSource ? useUpdateDataSource(dataSource.data_source_id) : null;
  ...
  const {
    ...
    watch,
    setValue,
  } = useForm<UpdateDataSourceForm>();

  // Populate form when dataSource changes
  useEffect(() => {
    if (dataSource && isOpen) {
      setValue('data_source_name', dataSource.data_source_name);
      setValue('data_source_description', dataSource.data_source_description || '');
      setValue('table_name', dataSource.table_name);
      setValue('schema_name', dataSource.schema_name);
      setValue('database_type', dataSource.database_type || 'postgresql');
      setValue('is_active', dataSource.is_active ?? true);
      setValue('requires_auth', dataSource.requires_auth ?? true);
    }
  }, [dataSource, isOpen, setValue]);
  ...
  const onSubmit = async (data: UpdateDataSourceForm) => {
    if (!dataSource || !updateDataSourceMutation) {
      setToastMessage('No data source selected for update');
      setToastType('error');
      setShowToast(true);
      return;
    }
                ...
                <DialogPanel className="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-xl shadow-xl">
                  ...
                  <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4">
                    ...
                    <div className="flex justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-700 mt-6">
                      ...
                      >
                        {isSubmitting ? 'Updating...' : 'Update Data Source'}
                      </button>
                    </div>
                  </form>
                </DialogPanel>
