...
export default function EndpointPerformanceTable({ metrics }: EndpointPerformanceTableProps) {
  const endpoints = Object.entries(metrics.performance.requests.byEndpoint).map(([endpoint, count]) => ({
    endpoint,
    requests: count,
    errors: metrics.performance.errors.byEndpoint[endpoint] || 0,
    errorRate: count > 0 ? ((metrics.performance.errors.byEndpoint[endpoint] || 0) / count) * 100 : 0,
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {endpoints.map((ep) => (
              <tr key={ep.endpoint} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="px-4 py-3 text-sm text-right text-gray-600 dark:text-gray-400">{ep.requests}</td>
                <td className="px-4 py-3 text-sm text-right text-gray-600 dark:text-gray-400">{ep.errors}</td>
                <td className="px-4 py-3 text-sm text-right">
                  <span className={`${ep.errorRate > 5 ? 'text-red-600' : ep.errorRate > 1 ? 'text-amber-600' : 'text-green-600'}`}>
