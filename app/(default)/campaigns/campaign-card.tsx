      ...
      <div className="flex flex-col h-full p-5">
        ...
        <footer className="mt-5">
          <div className="text-sm font-medium text-gray-500 mb-2">
            {campaign.dates.from} <span className="text-gray-400 dark:text-gray-600">-&gt;</span>{' '}
            {campaign.dates.to}
          </div>
          <div className="flex justify-between items-center">
            <div>
              <div
                className={`text-xs inline-flex font-medium rounded-full text-center px-2.5 py-1 ${typeColor(campaign.type)}`}
              >
                {campaign.type}
              </div>
            </div>
