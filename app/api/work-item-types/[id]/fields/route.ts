...
import { workItemFieldCreateSchema, workItemFieldsQuerySchema } from '@/lib/validations/work-item-fields';
 ...
 */
const postFieldHandler = async (request: NextRequest, userContext: UserContext, ...args: unknown[]) => {
 ...
 */
const getFieldsHandler = async (request: NextRequest, userContext: UserContext, ...args: unknown[]) => {
    const params = (args[0] as { params: { id: string } }).params;
    const startTime = Date.now();
    const workItemTypeId = params.id as string;
    ...
    try {
      ...
      const queryParams = {
        ...
        offset: searchParams.get('offset') ? parseInt(searchParams.get('offset') ?? '0', 10) : undefined,
      };
