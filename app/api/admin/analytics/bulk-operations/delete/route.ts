...
/**
 * POST /api/admin/analytics/bulk-operations/delete
...
const handler = rbacRoute(
  async (request: NextRequest, userContext) => {
    try {
      ...
      const operationId = await bulkChartOperationsService.bulkDeleteCharts(chartIds, userContext.user_id);
      ...
      console.error('Failed to start bulk delete operation:', error);
