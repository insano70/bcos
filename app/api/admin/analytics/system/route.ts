    ...
    const securityEvents = await db
      ...
      .where(
        and(
          ...
          eq(audit_logs.event_type, 'security')
        )
      )
...
function calculateSecurityScore(securityEvents: any[]): number {
  ...
  const criticalEvents = securityEvents.filter(e => e.severity === 'critical').reduce((sum, e) => sum + e.count, 0)
  const highEvents = securityEvents.filter(e => e.severity === 'high').reduce((sum, e) => sum + e.count, 0)
